...
<body>
  ...
  <main class="container my-5">
    ...
    <div class="d-flex align-items-center gap-2 mb-3">
      <button id="downloadPdf" class="btn btn-outline-primary">Download as PDF</button>
    </div>

    <div class="table-responsive">
      <table class="table table-hover align-middle table-bordered text-center w-100">
        <thead class="table-dark">
          <tr>
            <th class="w-25">ID</th>
            <th class="w-25">Handled By</th>
  ...
  <script>
          ...
          if (result.transactions.length > 0) {
            result.transactions.forEach((row) => {
              ...
              tbody.innerHTML += `
                <tr class="t_Row" data-transactionId="${row.id}">
                  ...
                  <td class="w-25">${row.cashier_username}</td>
                  <td class="w-25">${row.total}</td>
                  <td class="w-25">${row.date_added}</td>
                </tr>
            ...
            tbody.innerHTML += `
              <tr">
                ...
                <td colspan="2">₱${grandTotal.toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</td>
              </tr>
            `;
          } else {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      document.getElementById('downloadPdf').addEventListener('click', () => {
        ...
        let rangeText = '';
        if (start && end) rangeText = `Transactions from ${start} to ${end}`;
        else if (start) rangeText = `Transactions from ${start} onwards`;
        else if (end) rangeText = `Transactions up to ${end}`;
        else rangeText = `All Transactions`;

        // Create a temporary wrapper with the header + table
        ...
        const opt = {
          margin:       0.3,
          filename:     `transactions_${start || 'all'}_${end || 'all'}.pdf`,
