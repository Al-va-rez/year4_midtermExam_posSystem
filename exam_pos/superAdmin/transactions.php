...
<body>
  ...
  <main class="container my-5">
    ...
    <input type="date" id="searchUser" class="mb-3" placeholder="start date">
    <input type="date" id="searchUser" class="mb-3" placeholder="end date">
  ...
  <script>
    async function showTransactions(query = '') {
      try {
        const response = await fetch('../api.php', {
          ...
          body: JSON.stringify({ action: 'getTransactions', search: query })
        });
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // search users
      document.getElementById('searchUser').addEventListener('input', e => showTransactions(e.target.value.trim()));
